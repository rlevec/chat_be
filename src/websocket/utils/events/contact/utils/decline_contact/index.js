import { handleDeclineRequestAction } from "./utils/decline_request/index.js";
...
export const handleDeclineContactAction = async (
  io,
  userData,
  contactUsername
) => {
  ...
  const response = await handleDeclineRequestAction(
    userId,
    userData?.username,
    contactUsername,
    contactId
  ...
  const event = response?.success ? "decline_contact_request" : "error";
