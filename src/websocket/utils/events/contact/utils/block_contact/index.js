import { handleBlockRequestAction } from "./utils/block_request/index.js";
...
export const handleBlockContactAction = async (
  io,
  userData,
  contactUsername
) => {
  ...
  const response = await handleBlockRequestAction(
    userId,
    userData?.username,
    contactUsername,
    contactId
  ...
  const event = response?.success ? "block_contact_request" : "error";
